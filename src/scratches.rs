...
use crate::render::{blend_gray_pixel, write_images};

pub use crate::render::{RenderError, RenderSettings, Resolution, ResolutionParseError};
...
fn generate_images_with_rng<R: Rng + ?Sized>(
    ...
    settings.validate()?;
    write_images(&settings.render, "scratches", || {
        render_image(settings, rng)
    })
}
...
fn render_image<R: Rng + ?Sized>(settings: &ScratchSettings, rng: &mut R) -> RgbaImage {
    ...
    for effect in &settings.effects {
        effect.render(&mut image, settings.render.density, rng);
    }
    ...
    for y in start_y..=end_y {
        for x in start_x..=end_x {
            if point_in_polygon(x as f32 + 0.5, y as f32 + 0.5, &vertices) && !rng.random_bool(0.08)
            {
                ...
                blend_gray_pixel(image, x, y, shade, pixel_alpha);
                drew_pixel = true;
            }
        }
    }
